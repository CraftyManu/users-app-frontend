...
export function buildDashboardStats(users: DashboardUser[]): DashboardStats {
  ...
  users.forEach((user) => {
    const role = user.role.toUpperCase() as RoleId;
    roles[role]++;
    ...
    else if (age <= 25) ageGroups[1].count++;
    else if (age <= 35) ageGroups[2].count++;
    else if (age <= 45) ageGroups[3].count++;

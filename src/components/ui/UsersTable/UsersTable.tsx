...
export type SortKey = "nombre" | "email" | "edad" | "genero" | "localidad" | "role";
...
function UsersTable({ users, sortConfig, onSort, onView, onEdit, onDelete, canDelete, showDeleteButton }: UsersTableProps) {
        ...
        <tbody>
          {users.map((user) => (
            <tr key={user._id} className={styles.tr}>
              ...
              <td className={`${styles.td} ${styles.columnEmail}`}>{user.email}</td>

              <td className={`${styles.td} ${styles.columnEdad}`}>{user.edad}</td>

              <td className={`${styles.td} ${styles.genderCell} ${styles.columnGenero}`}>
                <GenderIcon gender={user.genero as "Masculino" | "Femenino"} />
              </td>

              <td className={`${styles.td} ${styles.columnLocalidad}`}>{user.localidad}</td>
              ...
              <td className={styles.td}>
                <div className={styles.actions}>
                  <button className={`${styles.iconBtn} ${styles.viewButton}`} onClick={() => onView(user)} title="Ver usuario">
                    <Eye size={18} />
                  </button>

                  <button className={`${styles.iconBtn} ${styles.editButton}`} onClick={() => onEdit(user)} title="Editar usuario">
                    <Pencil size={18} />
                  </button>

...
function UserCards({ users, onView, onEdit, onDelete, canDelete, showDeleteButton }: UserCardsProps) {
  if (users.length === 0) {
      ...
      {users.map((user) => (
        <article key={user._id} className={styles.card}>
          ...
          <footer className={styles.actions}>
            <button className={`${styles.iconButton} ${styles.viewButton}`} onClick={() => onView(user)} title="Ver usuario">
              <Eye size={18} />
            </button>

            <button className={`${styles.iconButton} ${styles.editButton}`} onClick={() => onEdit(user)} title="Editar usuario">
              <Pencil size={18} />
            </button>
